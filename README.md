...
This is a sophisticated but tiny & free alternative to legacy “CodeCleaner” utilities. It doesn't need installation; it works out-of-the box on 32/64-bit Office, no VBIDE references are required (late binding), handles mixed-locale CodeNames (`Sheet1`/`Ark1`), avoids `VERSION/Attribute` clutter in document modules, and can avoids issues with OneDrive/SharePoint file paths.
...
* **Stale compiled p-code**: the compiled stream inside modules can desync from source across Office versions or after heavy edits. VBA stores source code *and* compiled [p-code](https://en.wikipedia.org/wiki/Bytecode) inside the project. That p-code is not guaranteed portable across environments. Symptoms of incompatible p-code: 'phantom' compile errors that can appear and disappear, runtime errors, “Can’t find project or library,” or just flaky behavior that disappears after Debug → Compile VBAProject. You’ll need a recompile when, for example:
