...
This is a free alternative to legacy “CodeCleaner” utilities. It doesn'r need installation; it works out-of-the box on 32/64-bit Office, no VBIDE references are required (late binding), handles mixed-locale CodeNames (`Sheet1`/`Ark1`), avoids `VERSION/Attribute` clutter in document modules, and can avoids issues with OneDrive/SharePoint file paths.
