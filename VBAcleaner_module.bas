Attribute VB_Name = "VBAcleaner_module"
Option Explicit
Public Git_ProgName$ 'Program name. Set in GitSplashScreen_form.ProgName_label.Caption
Public Git_VersionID# 'Program version (integer=date, decimals=minor updates). Set in GitSplashScreen_form
...
'   - This macro should be run with VBA IDE option Tools > Options > General > Break on Unhandled Errors
...
Private Sub DeepClean_Workbook(Optional ByVal SrcWb As Workbook)
    ...
    For Each ws In src.Worksheets
        ...
        If i > 1 Then
            If ws.Visible = xlSheetVeryHidden Then
                ...
                ws.Copy After:=dst.Sheets(dst.Sheets.Count)
                ws.Visible = xlSheetVeryHidden
            Else 'visible or just hidden
                ws.Copy After:=dst.Sheets(dst.Sheets.Count)
            End If
        End If
    Next
    ...
    For i = 1 To UBound(refs, 1)
        ...
        If Err.Number <> 0 Then Debug.Print "Can't reinstate reference  " & 1 & ":", refs(i, 1), CLng(refs(i, 2)), CLng(refs(i, 3))
        On Error GoTo quitSub
    Next
...
Private Sub EnsureEmptyFolder(ByVal path$)
    ...
    If Len(Dir$(path, vbDirectory)) = 0 Then
        MkDir path
    ElseIf Dir(path & "\*.*") <> vbNullString Then
        Kill path & "\*.*"
    End If
    On Error GoTo 0
End Sub
...
Private Function LoadCodeBodyFromExport$(ByVal filePath$)
    ...
    Do While Not EOF(f)
        ...
        Else
            buf = buf & RTrim$(line) & vbCrLf 'Keep everything else (including Option Explicit, code, comments), and trim trailing spaces
        End If
    Loop
    Close #f
